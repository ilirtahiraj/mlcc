...
pcd_source_dir = '/media/ilirtahiraj/OSshared/diss/dissertation/03_paper/03_SensorToVehicle/carla_town01_rotation/scenario_1/'
...
for scene_num in range(1, 2):
    ...
    T_f_init = np.eye(4)
    #T_f_init[:3, :3] = R.from_euler('xyz', errors_f[:3], degrees=True).as_matrix()
    #T_f_init[:3, 3] = np.array(errors_f[3:])

    T_r_init = np.eye(4)
    T_r_init[:3, :3] = R.from_euler('xyz', errors_r[:3], degrees=True).as_matrix()
    T_r_init[:3, 3] = np.array(errors_r[3:])
    
    pcd_name = 1
    pcd_output_name = 0
    while pcd_name < 201:
        process_point_cloud("/media/ilirtahiraj/OSshared/diss/dissertation/03_paper/03_SensorToVehicle/carla_town01_rotation/scenario_1/lidar_front/" + str(pcd_name) + ".pcd",
                            T_f_init,
        ...
        #Use every 10th file
        pcd_name = pcd_name + 10
        pcd_output_name = pcd_output_name + 1

    pcd_name = 1
    pcd_output_name = 0
    while pcd_name < 201:
        process_point_cloud("/media/ilirtahiraj/OSshared/diss/dissertation/03_paper/03_SensorToVehicle/carla_town01_rotation/scenario_1/lidar_rear/" + str(pcd_name) + ".pcd",
                            T_r_init,
